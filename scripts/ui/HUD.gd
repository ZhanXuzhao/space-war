...
func on_player_ship_changed(new_ship: Node3D) -> void:
	...
	_player_controller = _find_player_controller(player_ship) if player_ship else null
	if player_ship:
		_connect_ship_signals()
...
func _on_btn_approach() -> void:
	...
	if _target_node is Node3D:
		if _player_controller and _player_controller.has_method("order_approach_target"):
			_player_controller.order_approach_target(_target_node)
		else:
